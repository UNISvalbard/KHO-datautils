...
Given a filename pointing to a Nikon D80 (DSLR) all-sky image, create a thumbnail image
with appropriate timestamp etc.

An example command line to go through the data (in the current working directory)
to create thumbnail images

find . -name *.jpg -exec python /home/mikkos/Nikon/Software/dslrThumbnail.py {} ;
...
def oneThumbnail(imagefile,overWriteExisting=False):
    ...
    filepattern=r"LYR-DSLR-(\d\d)(\d\d)(\d\d)_(\d\d)(\d\d)(\d\d).jpg"
    ...
    auroraXname=f'LYR-DSLR-{fileyear}{filemonth:02}{fileday:02}_{filehh:02}{filemm:02}{filess:02}.jpg'
    ...
    with Image.open(imagefile).resize((480,480)) as im:
        ...
        d.text((5,30),"DSLR", font=font1, fill=(255,255,255))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit("dslrThumbnail [filename]")

    imagefile=sys.argv[1]
    oneThumbnail(imagefile,overWriteExisting=False)
